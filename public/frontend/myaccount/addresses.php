    ...
    <div class="modal-content !max-w-[700px] !p-0">
        ...
        <form id="addressForm" class="p-12 space-y-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div class="space-y-3">
                    <label class="text-[10px] uppercase font-black tracking-widest text-gray-400">Address Type</label>
                    <select id="nodeName" required class="w-full h-14 bg-gray-50 border border-gray-100 px-6 text-sm font-bold focus:bg-white focus:border-gold outline-none transition-all">
    ...
    if (!user_id) {
        container.innerHTML = `
            <div class="col-span-full py-32 text-center bg-white border border-gray-100 flex flex-col items-center justify-center">
                <p class="text-[11px] uppercase tracking-[0.3em] text-gray-400 font-black mb-8 italic opacity-40">Authentication Required</p>
                <a href="<?= BASE_URL ?>login.php" class="text-[10px] uppercase font-black tracking-widest text-gold border-b border-gold/20 pb-1">Sign In to View Addresses</a>
            </div>`;
        return;
    }
    ...
    container.innerHTML = addresses.map((addr, idx) => `
        <div class="bg-white border border-gray-100 p-12 hover:border-gold hover:shadow-2xl transition-all duration-500 relative group">
            <div class="flex justify-between items-start mb-10">
                <span class="text-[9px] uppercase font-black tracking-[0.4em] text-gold italic">${addr.address_type === 'both' ? 'Billing & Shipping' : (addr.address_type === 'billing' ? 'Billing Only' : 'Shipping Only')}</span>
