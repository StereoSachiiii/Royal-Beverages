...
export function createEntityModule(config) {
    ...
    async function fetchSingle(id) {
        ...
        try {
            const res = await apiRequest(typeof apiRoutes.detail === 'function' ? apiRoutes.detail(id) : apiRoutes.detail.replace(':id', id));
            if (!res.success) throw new Error(res.message || `Failed to fetch ${entityName}`);
            return res.data;
        } catch (err) { throw err; }
    }
    ...
    function redrawTable(container, list) {
        container.querySelector('#entity-tbody').innerHTML = list.length ? list.map(renderRow).join('') : emptyRow(emptyMessage);
    ...
    function initFormHandlers(modalRoot, id, onSuccess) {
        ...
        if (delBtn && apiRoutes?.delete) {
            delBtn.addEventListener('click', async () => {
                ...
                try {
                    await apiRequest(typeof apiRoutes.delete === 'function' ? apiRoutes.delete(id) : apiRoutes.delete.replace(':id', id), { method: 'DELETE' });
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (form.querySelector('input[name="is_active"]')) payload.is_active = data.is_active !== undefined;
                if (form.querySelector('input[name="is_admin"]')) payload.is_admin = data.is_admin !== undefined;
                if (config.transformPayload) payload = config.transformPayload(payload, isEdit);

                const url = isEdit 
                    ? (typeof apiRoutes.update === 'function' ? apiRoutes.update(id) : apiRoutes.update.replace(':id', id)) 
                    : apiRoutes.create;
    ...
    async function Render() {
        ...
        const rows = _lastResults.length ? _lastResults.map(renderRow).join('') : emptyRow(emptyMessage);

        const subtitle = entitySubtitle || `${_lastResults.length} item${_lastResults.length === 1 ? '' : 's'} found`;
        ...
        if (sortOptions.length > 0) {
            sortEl.innerHTML = sortOptions.map(o => `<option value="${o.value}">${o.label}</option>`).join('');
    ...
    function Init(container) {
        ...
        const performSearch = debounce(async (q) => {
            _query = q; saveState(stateKeyQuery, _query); _offset = 0;
            const data = await fetchList(defaultLimit, 0, _query);
            _lastResults = Array.isArray(data) ? data : [];
            redrawTable(container, _lastResults);
            const subtitle = container.querySelector('.entity-subtitle');
            if (subtitle) subtitle.textContent = `${_lastResults.length} item${_lastResults.length === 1 ? '' : 's'} found`;
        }, 300);
        ...
        if (renderViewModal) {
            container.addEventListener('click', async (e) => {
                ...
                try {
                    ...
                    openStandardModal({ title: `${entityName.replace(/s$/, '')} Details`, bodyHtml: await renderViewModal(item), size: 'xl' });
                    ...
                    overlay?.addEventListener('click', async (me) => {
                        ...
                        if (editBtn && renderFormModal) {
                            ...
                            setTimeout(async () => {
                                ...
                                openStandardModal({ title: `Edit ${entityName.replace(/s$/, '')}`, bodyHtml: typeof f === 'string' ? f : f.firstElementChild?.outerHTML || f.outerHTML, size: 'xl' });
                                initFormHandlers(document.querySelector('.modal-overlay:last-child'), editBtn.dataset.id, reload);
                            }, 200);
                        }
                    });
                } catch (err) {
        ...
        if (renderFormModal) {
            container.addEventListener('click', async (e) => {
                ...
                try {
                    ...
                    openStandardModal({ title: `Edit ${entityName.replace(/s$/, '')}`, bodyHtml: typeof f === 'string' ? f : f.firstElementChild?.outerHTML || f.outerHTML, size: 'xl' });
                    initFormHandlers(document.querySelector('.modal-overlay:last-child'), btn.dataset.id, reload);
                } catch (err) {
                    openStandardModal({ title: 'Error', bodyHtml: `<p class="text-danger" style="padding:12px;">${escapeHtml(err.message)}</p>` });
                }
            }, { signal });
        ...
        if (_showCreate && renderFormModal) {
            container.addEventListener('click', async (e) => {
                ...
                try {
                    ...
                    openStandardModal({ title: `Create ${entityName.replace(/s$/, '')}`, bodyHtml: typeof f === 'string' ? f : f.firstElementChild?.outerHTML || f.outerHTML, size: 'xl' });
        ...
        container.addEventListener('click', async (e) => {
            ...
            if (list.length < defaultLimit) { btn.closest('#entity-load-more-container').style.display = 'none'; }
