...
async function fetchCarts(limit = 20, offset = 0, query = '') {
    try {
        const url = API_ROUTES.CARTS.LIST + buildQueryString({ limit, offset, ...(query ? { search: query } : {}) });
    ...
    } catch (err) { console.error('[Carts] Fetch failed', err); return []; }
}
    ...
    if (delBtn) {
        delBtn.addEventListener('click', async () => {
            if (!delBtn.dataset.confirmed) {
                ...
                setTimeout(() => { if (delBtn.isConnected) { delete delBtn.dataset.confirmed; delBtn.innerHTML = '🗑️ Delete Cart'; delBtn.classList.remove('btn-warning'); }}, 3000);
                return;
            }
            ...
            try {
                ...
                const msg = err.message.includes('foreign key') ? 'Cart is linked to an Order and cannot be deleted.' : err.message;
                showFormErrorFn(form, msg);
                delBtn.disabled = false; delBtn.innerHTML = '🗑️ Delete Cart';
            }
        });
    }
