...
function renderViewModal(sup) {
  const avgPrice = sup.avg_product_price_cents
    ? (sup.avg_product_price_cents / 100).toFixed(2)
    : '—';
  const isActive = sup.is_active !== false && sup.is_active !== 'f';

  const productsList =
    Array.isArray(sup.products) && sup.products.length
      ? sup.products
          .slice(0, 6)
          .map(
            (p) =>
  ...
  const moreCount =
    Array.isArray(sup.products) && sup.products.length > 6
