...
async function fetchPreferences(limit = 20, offset = 0, query = '') {
  try {
    const url =
      API_ROUTES.USER_PREFERENCES.LIST +
  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      if (!delBtn.dataset.confirmed) {
        ...
        delBtn.innerHTML = '⚠️ Confirm Reset?';
        delBtn.classList.add('btn-warning');
        setTimeout(() => {
          if (delBtn.isConnected) {
            delete delBtn.dataset.confirmed;
            delBtn.innerHTML = '🗑️ Reset Preferences';
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const cats = Array.from(
        form.querySelectorAll('input[name="favorite_categories"]:checked')
