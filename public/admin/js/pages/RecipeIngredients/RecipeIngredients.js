...
async function fetchIngredients(limit = 20, offset = 0, query = '') {
  try {
    const url =
      API_ROUTES.RECIPE_INGREDIENTS.LIST +
  ...
  if (pSel)
    pSel.innerHTML =
      ...
      products
        .map(
          (p) =>
            `<option value="${p.id}" ${p.id == i.product_id ? 'selected' : ''}>${escapeHtml(p.name)} (${p.sku || 'No SKU'})</option>`
        )
        .join('');
...
function initFormHandlersOverride(modalRoot, id, onSuccess, closeModalFn, showFormErrorFn) {
  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      if (!delBtn.dataset.confirmed) {
        ...
        delBtn.innerHTML = '⚠️ Confirm Remove?';
        delBtn.classList.add('btn-warning');
        setTimeout(() => {
          if (delBtn.isConnected) {
            delete delBtn.dataset.confirmed;
            delBtn.innerHTML = '🗑️ Remove Ingredient';
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const url = isEdit
        ? API_ROUTES.RECIPE_INGREDIENTS.UPDATE(id)
        : API_ROUTES.RECIPE_INGREDIENTS.CREATE;
