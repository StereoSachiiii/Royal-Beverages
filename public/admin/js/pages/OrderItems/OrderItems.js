...
async function fetchOrderItems(limit = 20, offset = 0, query = '') {
    try {
        const url = API_ROUTES.ORDER_ITEMS.LIST + buildQueryString({ limit, offset, ...(query ? { search: query } : {}) });
    ...
    } catch (err) { console.error('[OrderItems] Fetch failed', err); return []; }
}
...
function initFormHandlersOverride(modalRoot, id, onSuccess, closeModalFn, showFormErrorFn) {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (!submit.dataset.confirmed) {
            ...
            submit.classList.remove('btn-primary'); submit.classList.add('btn-warning');
        ...
        try {
            ...
            const payload = { quantity: parseInt(formData.get('quantity')), warehouse_id: formData.get('warehouse_id') ? parseInt(formData.get('warehouse_id')) : null };
